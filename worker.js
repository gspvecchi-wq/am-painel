...
const BOARDS = {
  ...
  Winners:  { id: '18392478822', tagCol: 'tag_mm3aj8fa',   cycleStartCol: 'text_mkyrgm3a',  cycleEndCol: 'text_mkyrq340',  statusCol: 'color_mkyrc9qa',  birthdayCol: 'text_mm3p2mt7',  fatInicialCol: null,               fatAtualCol: null,              dateFormat: 'DD/MM/YYYY' }
};
...
async function handleDados(request, env, ctx) {
  ...
  try {
    ...
    // Winners é uma turma própria — quadro independente, não se sobrepõe ao Master
    // fetchBoard com turma='Winners' já seta turma:'Winners' em cada item
    const masterNames = new Set(master.map(a => norm(a.name)));

    const allAlunos = [
      ...master,
      ...mentoria.filter(a => !masterNames.has(norm(a.name))),
      ...winnersRaw   // turma: 'Winners' — mantém como turma separada
    ];
...
async function fetchBoard(token, board, turma) {
  ...
  return items.map(item => {
    ...
    for (const col of (item.column_values || [])) {
      ...
      } else if (['phone','phone_1','phone0','telefone','phone_mkyh9wm6','phone_mkxfdrtf','phone_mm2fntym'].includes(col.id) && col.text) {
        phone = col.text;
